package com.hjy.sports.widget.dialog;
...
public abstract class BaseNiceDialog extends DialogFragment {
    ...
    private void initParams() {
        ...
        if (window != null) {
            ...
            //设置dialog宽度
            if (width == 0) {
                lp.width = ScreenUtils.getScreenWidth(getContext()) - 2 * DensityUtils.dp2px(getContext(), margin);
            } else {
                lp.width = DensityUtils.dp2px(getContext(), width);
            }
            //设置dialog高度
            if (height == 0) {
                lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
            } else {
                lp.height = DensityUtils.dp2px(getContext(), height);
            }
