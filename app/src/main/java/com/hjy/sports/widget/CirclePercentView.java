...
public class CirclePercentView extends View {
     ...
     * 默认的中心圆颜色
     */
    private static final int DEFAULT_HEART_COLOR_NORMAL = 0xFFffffff;
     ...
     * 默认的文本颜色（百分比为0）
     */
    private static final int DEFAULT_TEXT_COLOR = 0xFF797777;
     ...
     */
    private static final int DEFAULT_SKIN_INVALID_COLOR = 0xFFf1eeee;
    ...
    /**
     * 根据xml文件提供的属性值构建一个{@link CirclePercentView}对象。
     *
     * @param context Context上下文对象
     */
    @SuppressLint("NewApi")
    public CirclePercentView(Context context, AttributeSet attrs) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            mHeartClickAnimator = ObjectAnimator.ofFloat(CirclePercentView.this,
                    "BreathHeartPercent", 0f);
            mHeartClickAnimator.setDuration(DEFAULT_CLICK_ANIMATION_LENGTH);
            mArrivePercentAnimator = ObjectAnimator.ofFloat(CirclePercentView.this,
                    "Percent", 0f);
            mArrivePercentAnimator.addListener(new AnimatorListenerAdapter() {

                @Override
                public void onAnimationEnd(Animator animation) {
                    CirclePercentView.this.setLayerToSW(CirclePercentView.this);
                    if (mOnPercentAnimationEndListener != null) {
                        mOnPercentAnimationEndListener
                                .onPercentAnimationEnd(CirclePercentView.this);
                    }
                }
            });
        }
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.CirclePercentView, 0, 0);
        try {
            mTextColor = a.getColor(R.styleable.CirclePercentView_centerTextColor,
                    DEFAULT_TEXT_COLOR);
            mHeartColor = a.getColor(R.styleable.CirclePercentView_heartColor,
                    DEFAULT_HEART_COLOR_NORMAL);
            mHeartClickedColor = a.getColor(
                    R.styleable.CirclePercentView_heartClickedColor,
                    DEFAULT_HEART_COLOR_CLICKED);
            setAnimationDuration(a.getInt(R.styleable.CirclePercentView_duration,
                    (int) DEFAULT_ANIMATION_LENGTH));
            if (!a.hasValue(R.styleable.CirclePercentView_skinStartColor)
                    ^ a.hasValue(R.styleable.CirclePercentView_skinEndColor)) {
                mSkinStartColor = a.getColor(
                        R.styleable.CirclePercentView_skinStartColor,
                        DEFAULT_SKIN_START_COLOR);
                mSkinEndColor = a.getColor(
                        R.styleable.CirclePercentView_skinEndColor,
                        DEFAULT_SKIN_END_COLOR);
            } else if (a.hasValue(R.styleable.CirclePercentView_skinStartColor)) {
            ...
            mSkinDepth = a.getInt(R.styleable.CirclePercentView_skinDepth,
                    DEFAULT_SKIN_DEPTH);
            mPercent = a.getFloat(R.styleable.CirclePercentView_percent, 0f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Float.compare(mPercent, 0) >= 0) {
            ...
            canvas.drawArc(mBounds, -90 + radianAngle, 360 - radianAngle,
                    false, mSkinPaint);
            String percentText = mPercentFormat.format(textmPercent);
...
//                    mTextPaint);
                        ...
                        if (alwaysKeepInHeart && pointInLemonHeart(x, y)) {
                            ...
                            if (mOnHeartClickListener != null) {
                                mOnHeartClickListener
                                        .onHeartClick(CirclePercentView.this);
                            }
                        }
                        alwaysKeepInHeart = false;
                        return true;
    ...
    private Boolean pointInLemonHeart(float x, float y) {
        ...
        return Float.compare((float) Math.sqrt(xx + yy), r) < 0;
    }
    ...
    public interface OnPercentAnimationEndListener {
         ...
         * 当百分比动画播放结束时被调用
         *
         * @param lemon PercentLemon对象
         */
        void onPercentAnimationEnd(CirclePercentView lemon);
    }
    ...
    public interface OnHeartClickListener {
         ...
         * @param lemon PercentLemon对象
         */
        void onHeartClick(CirclePercentView lemon);
    }
    ...
    private int getCurrentColor(int startColor, int endColor, float percent) {
        ...
        return (startA + (int) (percent * (endA - startA) / 100)) << 24
                | (startR + (int) (percent * (endR - startR) / 100)) << 16
                | (startG + (int) (percent * (endG - startG) / 100)) << 8
                | (startB + (int) (percent * (endB - startB) / 100));
    }
     ...
     * 设置中心圆点击状态的颜色值
     *
     * @param heartClickedColor 中心圆点击颜色
     ...
     *
     * @param skinDepth 外围圆环像宽度比重
     * @throws IllegalArgumentException 如果传递的参数值不在1到50之间
     ...
     *
     * @param percent 百分比值0到100之间
     * @throws IllegalArgumentException 如果传递的参数值不在0到100之间
...
//            throw new IllegalArgumentException("百分比值必须在0到100之间");
//        }
//        if (Float.compare(toValue, 100) > 0) {
//            toValue = 100;
//        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            setLayerToHW(this);
            stopChangingPercent();
            if (Float.compare(toValue, mPercent) != 0) { //0： 为相等 1： 前面的大于后面的数据  -1：后面大于前面
