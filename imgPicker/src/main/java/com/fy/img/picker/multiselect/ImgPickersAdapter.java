    ...
    public void convert(ViewHolder holder, ImageItem imgItem, int position) {
        if (position == 0 && imgItem.isShowCamera() == 0){//判断 是否显示 拍照按钮
            holder.setOnClickListener(R.id.camera, v -> {
                String takeImgFilePath = FileUtils.getPath("/DCIM/camera/");
                ImagePicker.newFilePath = FileUtils.createFile(takeImgFilePath, "IMG_", ".png");
                //拍照
                PhotoUtils.takePicture((Activity) mContext, ImagePicker.newFilePath, ImagePicker.Photograph);
            });
        } else {
